...
import { requestSaveConfigToFile, requestUploadConfigToDevice } from "@features/config/configActions";
import {
...
function* saveConfigToFileWorker(action: ReturnType<typeof requestSaveConfigToFile>) {
  try {
...
function* uploadConfigToDeviceWorker(action: ReturnType<typeof requestUploadConfigToDevice>) {
  try {
    yield put(requestSliceActions.setRequestPending({ name: action.type }));
...
export function* configSaga() {
  ...
  yield all([takeEvery(requestUploadConfigToDevice.type, uploadConfigToDeviceWorker)]);
}
